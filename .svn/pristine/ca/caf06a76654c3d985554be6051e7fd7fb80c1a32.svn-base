...
import com.goldenapple.lottery.util.ColorUtils;
...
public class SwitchButton extends CompoundButton {
	...
	private void init(AttributeSet attrs) {
		...
		Drawable thumbDrawable = null;		/** 前景样式 */
		ColorStateList thumbColor = null; 	/** 前景颜色 */
		...
		Drawable backDrawable = null;		/** 背景样式 */
		ColorStateList backColor = null; 	/** 背景颜色 */
		...
		if (ta != null) {
			...
			textOffColor = ta.getColor(R.styleable.SwitchButton_kswTextOffColor, Color.GRAY);
			textOn = ta.getString(R.styleable.SwitchButton_kswTextOn);
			textOnColor = ta.getColor(R.styleable.SwitchButton_kswTextOnColor, Color.RED);

			textSize = ta.getDimension(R.styleable.SwitchButton_kswTextSize, 24);
	...
	protected void onDraw(Canvas canvas) {
		...
		if(textOff!=null&&textOn!=null){
			...
			if(isChecked()) {
				mPaint.setColor(textOnColor);
				canvas.drawText(textOn, mPaint.getTextSize() / 2, (this.getHeight() + mPaint.getTextSize()) / 2, mPaint);
			}else {
				mPaint.setColor(textOffColor);
				canvas.drawText(textOff, (this.getWidth() - mPaint.measureText(textOff)) - mPaint.getTextSize() / 2, (this.getHeight() + mPaint.getTextSize()) / 2, mPaint);
			}
		}
