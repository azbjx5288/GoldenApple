...
public class CountdownView extends View {
    ...
    private void initTimeTextBaselineAndTimeBgTopPadding() {
        if (getPaddingTop() == getPaddingBottom()) {
            // center
            mTimeTextBaseline = (mViewHeight + mTimeTextHeight) * 0.5f - mTimeTextBottom;

            mTopPaddingSize = (mViewHeight - mContentAllHeight) * 0.5f;
        } else {
            // padding top
    ...
    private float getSuffixTextBaseLine(String suffixText) {
        ...
        switch (mSuffixGravity) {
            ...
            case 1:
                // center
                if (isHideTimeBackground) {
                    ret = mTimeTextBaseline - (mTimeTextHeight - tempRect.height()) * 0.5f;
                } else {
                    ret = mTopPaddingSize + mTimeBgSize - (mTimeBgSize - tempRect.height()) * 0.5f;
                }
                break;
            case 2:
                // bottom
    ...
    private void initLeftPaddingSize() {
        if (getPaddingLeft() == getPaddingRight()) {
            // center
            mLeftPaddingSize = (mViewWidth - mContentAllWidth) * 0.5f;
        } else {
            // padding left
